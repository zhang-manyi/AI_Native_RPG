...
- **结局**：至少 3 个，否则是技术演示而不是故事——查明真相 / 被洛伦先动手 / 玛尔塔彻底闭口（fear 过高，社交线断掉）。切片 5 把这三个倒推成结构化成立条件，并各附一条可达路径（[14 §4](./14_Case_Design.md#4-三个结局)）。三条现在写在剧本的 `narrative.endings` 里并由加载器校验可达；"被洛伦先动手"标着 `pending`，因为抬 tension 的 M6 还没写。`fear` 已经会涨了（见下方第二批）。
