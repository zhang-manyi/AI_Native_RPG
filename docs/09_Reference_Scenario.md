...
- **结局**：至少 3 个，否则是技术演示而不是故事。剧本的 `narrative.endings` 声明成立条件，加载器检查其写入通道。**四个终局均已有可达路径**：查明真相 / 指控错人 / 被洛伦先动手 / 没查明白（天数用尽）。本轮补齐 M6 最小分支并移除 `loren_moves_first.pending`，正常开局验收见 §4。「玛尔塔彻底闭口」仍是非终局里程碑（`terminal: false`），不单列为已验收的独立结局；玛尔塔目击线关闭后可继续独立调查的路径另有回归覆盖。
...
**2026-09-18真实整局验收补充**：首次路线到达终局但因私密情节披露未通过，
[失败实录](../evals/reports/2026-09-18-demo-acceptance/review.md)保持不变。
随后修复公开表达输入边界，原路线与冻结变体均从普通开局到`truth_uncovered`，
通过本轮有限验收；存档恢复、拒绝移动后继续及终局限制通过。
真实Web/终端默认采用新边界，旧表达候选关闭。仍有表达含混、角色回避和模型编造风险，
不宣称整体安全；[修复证据](../evals/reports/2026-09-18-public-expression/review.md)。
...
| 6 | 最小闭环完成；两条真实引导路线通过有限验收 | 基础6轮、记忆与表达对照、工具任务；完整路线及冻结变体验证 | 新公开表达边界用于真实入口；三个旧候选仍关闭，广泛模型质量未验收，见 [实验总览](../evals/OVERVIEW.md) |
...
- **结局可达性由加载器校验**（[13 §11](./13_Narrative_Events.md#11-每个结局必须可达)）。剧本用 `narrative.endings` 声明结局，`reachability.py` 在启动时检查路径及写入者；关系值按 `(npc, dimension)` 成对追踪。历史上它抓到了张力终局没有写入通道的问题，曾用 `pending: true` 保留未实现声明。M5 后来提供了单次张力写入，但仍不足以达到门槛；本轮 M6 补齐累积风险及警告披露后移除 `pending`，并通过正常开局测试证明路径可达。

**待办核对与本轮交付（2026-09-16，基于 `14fee40` 工作区）**：仅核对旧待办与本次必要回归，未重新全面审查切片 1–3。当前 Web 执行约定见 [16](./16_Guided_Playthrough.md)，13–15 保留的完整设计不等于当前已实现范围。

| 旧待办 | 当前结论 | 代码依据与边界 |
|---|---|---|
| Web 时段制、移动、多 NPC 对话路由 | 已实现 | `web/session.py`、`routes_player.py`；按当前位置选择独立 NPC runtime，移动扣时段，收束后过夜 |
| M3/M4/M5、F2/F3、M7 | 已实现当前引导式版本 | `world.yaml` 的事件、条件与结果；M7 由玩家主动开启，真相选项要求 F1/F2/F3 和森林足迹 |
...
0.6 时必须先听取警告才能继续走危险选项；“停止施压”永久关闭这个施压事件，仍可根据已有线索作结论。M6 转述只发生一次，用 `reverse` 标签表达玛尔塔由“包庇嫌疑”到“护子”的重读；不移动洛伦，不做日程，不把他对那晚的误解写进玩家刚听到的威胁线索。场外警告的 fear 变化与披露在玩家回访时一并结算。

**验证边界**：`tests/test_web_playthrough.py` 用普通 Session 和玩家 HTTP 路由走到新终局，不直接写 trust/stage/tension/flags；覆盖警告后停止施压仍可查明真相、单次警告、过期选项拒绝、Trace 中的 Validator 裁决、终局存读档及后续动作拒绝。真相、早期指认、超时、玛尔塔目击线关闭后的独立调查路径保留。上述为离线/mock 契约与确定性路径验证；本轮未调用真实模型，不据此宣称自由文本分类、台词质量或记忆质量已通过真实模型验收。
...
**截至 2026-09-18，工具决策与短任务闭环也已完成，候选不采用**：新开发/留出各6个完整任务（含两步NPC移动）和2个局部拒绝控制，每条件三次真实重复。根据开发失败只补公开事实ID约束，留出主要参数指标不改善、任务成功15/18→13/18，完整指定拒绝均0/6；两个旧表达候选仍关闭。结果与分母见 [实验总览](../evals/OVERVIEW.md)、[工具报告](../evals/reports/2026-09-18-tools/review.md) 和 [旧受限表达报告](../evals/reports/2026-09-17-expression/review.md)。下一步优先收紧已有资料到最终表达的依据与披露边界，并设计符合角色约束、能真实触发的任务/拒绝路径；不以多查工具或合法状态替代玩家目标。若使用本次留出失败调优，应转为开发资料并准备新留出，保留失败和成本，没有证据不推广。Player Model 继续后置；暂不增加剧情、完整平台或自动搜索。facts 整段仍只是部分实现，四个结局保持回归覆盖。

**2026-09-18 后续证据交接闭环**：只修行动后重写丢失已取得资料这一因素，新增默认关闭的grounded_rewrite。查到“艾拉十四岁”后开发回答0/3→3/3，固定行动后回述物名在新留出0/6→6/6；完整新留出仍有4/15披露、指定拒绝0/6，因此保留基线。96输出全部保存并逐条助手复核，953项全仓库测试通过、6跳过，后增汇总测试3通过。详见 [报告](../evals/reports/2026-09-18-answer-evidence/review.md)。没有启用两个旧表达候选，没有扩展剧情或Player Model。
