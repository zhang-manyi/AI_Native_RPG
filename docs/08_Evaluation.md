...
最新执行结果见 [行动后证据交接实验](../evals/reports/2026-09-18-answer-evidence/review.md)：仅补原重写请求的已取得可用证据，开发普通答案4/9→9/9、新留出8/9→9/9，局部物名0/6→6/6；仍有披露和完整拒绝覆盖不足，因此grounded_rewrite默认关闭。旧 [受限表达证据实验](../evals/reports/2026-09-17-expression/review.md) 的两个开关也仍关闭；本轮用过的旧留出明确降为开发数据。历史报告保留，语义结论为逐条助手复核，未实现通用Judge。
...
2026-09-18 已完成一次 [工具决策/短任务评测](../evals/TOOL_TASKS.md)：新开发/留出划分、
每条件三次真实重复、开发失败只改公开事实ID查询约束。留出未证明改善，保留基线，
见 [报告](../evals/reports/2026-09-18-tools/review.md)。上文 Tool Use Success Rate
只说明执行契约；新评测分别记录工具选择、参数、执行、行动选择、真实拒绝覆盖、
状态与任务达成。`check_public_fact` 对错误ID返回 known=false 也能正常执行，不能算选对参数。
两步NPC移动用真实前状态；主动拒绝不算提交后被拒，局部固定行动拒绝不算完整覆盖。
语义指标是助手复核，不是上文尚未实现的通用 Judge。
