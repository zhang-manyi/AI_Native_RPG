"""Contract tests for DeepSeekClient, served entirely by httpx.MockTransport.
...
from ai_native_rpg.llm.deepseek import DeepSeekAPIError, DeepSeekClient
...
def _client(handler, **kwargs) -> DeepSeekClient:
...
def _ask(client: DeepSeekClient, **kwargs):
...
class TestReparseRetry:
    ...
    def test_gives_up_after_the_reparse_retry(self):
        ...
        with pytest.raises(DeepSeekAPIError, match="schema"):
            _ask(_client(handler))
...
class TestTransportRetries:
    ...
    def test_timeout_exhausted_raises(self):
        ...
        with pytest.raises(DeepSeekAPIError, match=r"timed out|timeout"):
