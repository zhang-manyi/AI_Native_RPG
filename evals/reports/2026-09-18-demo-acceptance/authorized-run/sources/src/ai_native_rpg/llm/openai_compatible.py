"""OpenAICompatibleClient: the real backend behind the ``LLMClient`` Protocol.
...
the only seam and ``httpx`` is already a dependency. Two payoffs, one expected and
one not:

* testability — ``httpx.MockTransport`` lets the whole request/retry/parse path
  run offline against the bytes that would really go on the wire;
* provider portability — DeepSeek, OpenAI and OpenAI-compatible relays are the
  same protocol, so adding one costs an entry in ``config._PROVIDERS`` and no code
  here. A vendor SDK would have made each a separate implementation of this file.
  Hence the neutral name: it was ``DeepSeekClient`` while DeepSeek was the only
  backend, which read as a vendor binding it never actually had.
...
class OpenAICompatibleClient:
    ...
    Defaults point at DeepSeek because that is this project's usual backend; pass a
    different ``base_url``/``model`` for OpenAI or a relay.
    ...
    def __repr__(self) -> str:
        ...
        return f"OpenAICompatibleClient(base_url={self._base_url!r}, model={self._model!r})"
    ...
    def complete(
        ...
        *,
        ...
        except LLMAPIError as first_error:
        ...
        for attempt in range(1, self._max_attempts + 1):
            try:
                ...
                if response.status_code not in _RETRY_STATUSES:
                    ...
                    # delays the error.
                    raise LLMAPIError(f"upstream request failed: {last_error}")

            if attempt < self._max_attempts:
                self._sleep_backoff(attempt)

        raise LLMAPIError(
            f"upstream request failed after {self._max_attempts} attempts: {last_error}"
        )
    ...
    def _message(raw: dict[str, Any]) -> dict[str, Any]:
        ...
        if not choices:
            raise LLMAPIError("upstream response contained no choices")
    ...
    def _extract_tool_calls(self, raw: dict[str, Any]) -> list[ToolCall]:
        ...
        for item in self._message(raw).get("tool_calls") or []:
            ...
            try:
                # OpenAI-compatible APIs send arguments as a JSON *string*; decoding it here
    ...
    def _parse(content: str, schema: type[BaseModel]) -> BaseModel:
        ...
        if not text.strip():
            raise LLMAPIError(f"empty response body, expected {schema.__name__} schema")
        try:
