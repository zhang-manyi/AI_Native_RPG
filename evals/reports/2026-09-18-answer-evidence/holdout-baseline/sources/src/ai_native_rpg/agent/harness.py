...
from .expression import assemble_expression_evidence, assemble_rewrite_evidence
    ...
    def respond(
        ...
        *,
        ...
        if resolved_outcome is not None:
            if self._filtered_dialogue_evidence:
        ...
        if planning.action is None:
            ...
            if self._filtered_dialogue_evidence and resolved_outcome is None:
                started = time.perf_counter()
                ...
                steps.append(
                    TraceStep(
                        step_name="dialogue_generation",
                        output_summary={"regenerated": True, "filtered_evidence": True},
                        latency_ms=(time.perf_counter() - started) * 1000,
                        model_used=reply.model,
                        token_usage=reply.token_usage,
                    )
        ...
        *,
        ...
        steps.append(
            TraceStep(
                ...
                input_summary={"action_status": action_status},
                output_summary=evidence,
            )
        )
        return [
            Message(role="system", content=self._prompts.load("npc_expression.txt")),
            Message(
                role="user",
                content=json.dumps(
                    {
