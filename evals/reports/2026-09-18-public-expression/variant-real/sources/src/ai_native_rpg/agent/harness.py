...
from .expression import (
    ResolvedPublicOutcome,
    assemble_expression_evidence,
    assemble_rewrite_evidence,
...
class Harness:
    ...
    def __init__(
        ...
        *,
        ...
        retain_dialogue_evidence: bool = False,
        filtered_dialogue_evidence: bool = False,
        grounded_rewrite: bool = False,
        public_expression: bool = False,
    ) -> None:
        self._npc = npc_state
        ...
        # Experimental context variant; fixed-suite evaluation has not justified promotion.
        self._retain_dialogue_evidence = retain_dialogue_evidence
        if retain_dialogue_evidence and filtered_dialogue_evidence:
            raise ValueError("raw and filtered evidence variants are mutually exclusive")
        self._filtered_dialogue_evidence = filtered_dialogue_evidence
        if grounded_rewrite and (retain_dialogue_evidence or filtered_dialogue_evidence):
            raise ValueError("grounded rewrite must be evaluated separately from older variants")
        self._grounded_rewrite = grounded_rewrite
        if public_expression and (
            retain_dialogue_evidence or filtered_dialogue_evidence or grounded_rewrite
        ):
            raise ValueError("public expression must be evaluated separately from older variants")
        self._public_expression = public_expression
    ...
    def respond(
        ...
        *,
        ...
        resolved_public_outcome: ResolvedPublicOutcome | None = None,
        ...
        if resolved_outcome is not None:
            if self._public_expression:
                if resolved_public_outcome is None:
                    raise ValueError(
                        "public expression requires an explicit resolved public outcome"
                    )
                messages = self._expression_messages(
                    observation,
                    retrieval,
                    steps,
                    player_id,
                    action_status="resolved",
                    resolved_public_outcome=resolved_public_outcome,
                )
            elif self._filtered_dialogue_evidence:
                messages = self._expression_messages(
                    observation, retrieval, steps, player_id, action_status="resolved"
                )
            else:
                messages = self._planning_messages(observation, retrieval, player_id)
            if not self._public_expression:
                messages.append(
                    Message(
                        role="system",
        ...
        if planning.action is None:
            ...
            if (
                self._filtered_dialogue_evidence or self._public_expression
            ) and resolved_outcome is None:
                started = time.perf_counter()
                reply = self._llm.complete(
                    self._expression_messages(observation, retrieval, steps, player_id),
                ...
                steps.append(
                    TraceStep(
                        ...
                        output_summary={
                            "regenerated": True,
                            "filtered_evidence": self._filtered_dialogue_evidence,
        ...
        self._reflect(observation, dialogue, npc_id, steps, player_id=player_id)
    ...
    def remember_exchange(
        self, observation: str, dialogue: str, *, player_id: str | None = None
    ) -> None:
        """Keep authored exchanges in this NPC's memory without a model call."""
        self._reflect(observation, dialogue, self._npc.npc_id, [], player_id=player_id)
                    ...
                    "episodic": [
                        {
                            ...
                            "source_player_id": m.source_player_id,
                            "player_statement": m.player_statement,
                        }
    ...
    def _plan(
        ...
        for iteration in range(self._max_tool_iterations + 1):
            ...
            if resp.wants_tools and offer_tools is not None:
                steps.append(
                    TraceStep(
                        step_name="tool_request",
                        output_summary={"tool_count": len(resp.tool_calls)},
    ...
    def _run_tool(self, call: ToolCall, steps: list[TraceStep]) -> Message:
        ...
        try:
            ...
            summary = {"ok": True, "result_keys": sorted(result), "result": result}
    ...
    def _act_and_regenerate(
        ...
        *,
        ...
        start = time.perf_counter()
        evidence = {
            "episodic": [m.event_description for m in retrieval.episodic],
            "semantic": [m.fact for m in retrieval.semantic],
            "工具返回": [
        ...
        if self._filtered_dialogue_evidence or self._public_expression:
            messages = self._expression_messages(
                observation,
                retrieval,
                steps,
                player_id,
                action_status="applied" if result.approved else "rejected",
                action_result=(
                    public_action_result(self._manager, player_id, proposal, result)
                    if self._public_expression
                    else None
                ),
            )
        else:
            messages = self._dialogue_messages(
                plan,
                result.reason,
                observation,
                narrative_event,
                evidence=evidence if self._retain_dialogue_evidence else None,
            )
            if self._grounded_rewrite:
                supplement = assemble_rewrite_evidence(
                ...
                steps.append(TraceStep(step_name="rewrite_evidence", output_summary=supplement))
                if supplement["public_facts"] or supplement["player_statements"]:
                    messages.extend(
                        [
                            Message(
                                role="system",
                                content=self._prompts.load("npc_rewrite_evidence.txt"),
                            ),
                            Message(
                                role="user", content=json.dumps(supplement, ensure_ascii=False)
                            ),
                        ]
                    )
        ...
        *,
        ...
        self._memory.add_episodic(
            EpisodicMemory(
                ...
                player_statement=self._clip_reflection(observation) if player_id else None,
            )
        )
        steps.append(
            TraceStep(
    ...
    @staticmethod
    def _clip_reflection(text: str) -> str:
        text = text.strip()
        return text if len(text) <= _REFLECTION_HALF_LIMIT else text[:_REFLECTION_HALF_LIMIT] + "…"

    @classmethod
    def _reflection_text(cls, observation: str, dialogue: str) -> str:
        ...
        return (
            f"玩家说：{cls._clip_reflection(observation)}；我回应：{cls._clip_reflection(dialogue)}"
        )

    # --- prompt assembly ---------------------------------------------------

    def _expression_messages(
        self,
        observation: str,
        retrieval: MemoryRetrievalResult,
        steps: list[TraceStep],
        ...
        *,
        ...
        evidence = assemble_expression_evidence(
            manager=self._manager,
            memory=self._memory,
            retrieval=retrieval,
            steps=steps,
        ...
        if self._public_expression:
            ...
            if resolved_public_outcome is not None:
                evidence["resolved_outcome"] = resolved_public_outcome.model_dump()
        public_npc = self._manager.snapshot().npcs.get(self._npc.npc_id)
        identity = {
            "name": public_npc.display_name if public_npc else self._npc.npc_id,
            "public_note": public_npc.public_note if public_npc else "",
            "traits": self._npc.persona.traits,
        }
        # Private background/goal/reasoning and free-form strategy can all contain
        # secrets. Never copy them into this expression-only call, even on rejection.
        steps.append(
            TraceStep(
                step_name="expression_evidence",
                input_summary={
                    "action_status": action_status,
                    "public_expression": self._public_expression,
                },
        ...
        return [
            Message(
                role="system",
                content=self._prompts.load(
                    "npc_public_expression.txt" if self._public_expression else "npc_expression.txt"
                ),
            ),
            Message(
                role="user",
        ...
        *inner* voice, which necessarily speaks of the player in the third person
        ...
        if evidence is not None:
            evidence_block = (
                ...
                + "\n记忆是主观认知，不等于公开事实或披露许可。"
                "工具结果是行动前的查询记录；本轮校验结果优先。"
                "回答应沿用这些依据，不要编造或替换已经查明的名字和细节。"
            )
        return [
            Message(role="system", content=f"{system}\n\n{self._persona_block()}"),
            Message(
                ...
                content=(
                    ...
                    f"校验结果：{verdict}{self._narrative_block(narrative_event)}{evidence_block}"
                ),
            ),
        ]
