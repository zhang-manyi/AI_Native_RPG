...
def snapshot(game):
    ...
    return {
        ...
        "wrap_up": wrap_up.model_dump(mode="json") if wrap_up is not None else None,
    }
...
def run(route_path, output, *, public_expression=None):
    output.mkdir(parents=True, exist_ok=False)
    settings = Settings.from_env()
    if public_expression is not None:
        settings = settings.model_copy(update={"public_expression": public_expression})
    ...
    report = {
        ...
        "candidate_flags": {
            ...
            "public_expression": settings.public_expression,
        },
        "steps": [],
        "completed": False,
    }
    ...
    try:
        with (
            ...
            report["runtime_flags"] = {
                npc_id: {
                    key: getattr(runtime.harness, "_" + key) for key in report["candidate_flags"]
                }
                for npc_id, runtime in game._npcs.items()
            }
            ...
            report["expression_fallbacks"] = sum(
                step["step_name"] == "expression_fallback"
                for path in (output / "traces").glob("*.json")
                for step in json.loads(path.read_text(encoding="utf-8"))["steps"]
            )
            if report["expression_fallbacks"]:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        help="explicitly select the new expression boundary; omitted uses game settings",
    )
    args = parser.parse_args()
    raise SystemExit(run(args.route, args.output, public_expression=args.public_expression))
